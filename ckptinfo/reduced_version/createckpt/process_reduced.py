...
def combine1(loadinfo, start, size):
    saddr = loadinfo[start].addr
    eaddr = saddr + size
    page_bound = saddr - (saddr%4096) + 4096
    res = []
    data = 0
    if eaddr > page_bound:
        res.append(-1)
        return res
    idx = start
    # print('combine1: 0x%x '%saddr, end="")
    while idx < len(loadinfo):
        if loadinfo[idx].addr < eaddr:
            shift = loadinfo[idx].addr - saddr
            data = data + ((loadinfo[idx].data) << (shift*8))
            # print('(0x%x'%loadinfo[idx].data + ' %d) '%shift, end="")
        else:
            break
        idx = idx + 1
    res.append(idx)
...
def combine(loadinfo):
    ...
    while idx < len(loadinfo):
        ...
        if idx + 7 < len(loadinfo) and loadinfo[idx+7].addr == saddr + 7:
            ...
            res = combine1(loadinfo, idx, 4)
            if res[0] != -1:
                infos.append(Memdata(loadinfo[idx].addr, res[1], res[2]))
                idx = res[0]
                continue
            if idx + 1 < len(loadinfo) and loadinfo[idx+1].addr == saddr + 1:
                step = 2
            else:
                step = 0
...
def writeLoadInfo(f1, loadinfo):
    ...
    idx = 0
    while idx < len(loadinfo):
        if loadinfo[idx].data == 0:
            zeroloads.append(loadinfo[idx])
        elif loadinfo[idx].size == 1:
            loads1.append(loadinfo[idx])
        elif loadinfo[idx].size == 2:
            loads2.append(loadinfo[idx])
        elif loadinfo[idx].size == 4:
            loads4.append(loadinfo[idx])
        else:
            loads8.append(loadinfo[idx])
        idx = idx + 1

    print(len(zeroloads), len(loads8), len(loads4), len(loads2), len(loads1))

    f1.write(toBytes(len(zeroloads), 4))    ## write number
    for load in zeroloads:
        f1.write(toBytes(load.addr, 4))
        f1.write(toBytes(load.size, 4))

    f1.write(toBytes(len(loads8), 4))    ## write number
    for load in loads8:
        f1.write(toBytes(load.addr, 4))
        f1.write(toBytes(load.data, 8))

    f1.write(toBytes(len(loads4), 4))    ## write number
    for load in loads4:
        f1.write(toBytes(load.addr, 4))
        f1.write(toBytes(load.data, 4))

    f1.write(toBytes(len(loads2), 4))    ## write number
    for load in loads2:
        f1.write(toBytes(load.addr, 4))
        f1.write(toBytes(load.data, 4))

    f1.write(toBytes(len(loads1), 4))    ## write number
    for load in loads1:
        f1.write(toBytes(load.addr, 4))
        f1.write(toBytes(load.data, 4))
...
def writefile(startpoint, syscallinfos, loadinfo, memrange, textinfo, end, exitinfo):
    ...
    ckptname = bench+"_rdc_ckpt_"+str(startpoint.inst_num)+".info"
    syscallname = bench+"_rdc_ckpt_syscall_"+str(startpoint.inst_num)+".info"
