...
if [[ "$ARGC" < 4 ]]
then
    echo "./xxx.sh 400.perlbench(400, perlbench) number(ref=1,2...) (arm riscv mips alpha amd64) checkpoint(1,2,3)"
    exit
fi
...
spec_dir=/root/spec2006
checkpoint_dir=/root/ckpt_spec2006
checkpoint_dir=$checkpoint_dir/checkpoint_$ISA

gem5_dir=/home/gem5_src/gem5g
...
gem5_args="--cpu-type=DerivO3CPU  --mem-size=8GB --l1d_size=64kB --l1d_assoc=8 --l1i_size=32kB --l1i_assoc=4 --l2cache --l2_size=2MB --l2_assoc=16"
...
#find real checkpoint Number
function find_realNum(){
    num=1
    for str in `cat $checkpoint_dir/cpt.record`
    do
...
#确定运行参数
checkpoint_dir=$checkpoint_dir/$target/$ISA-$target-"input"$2
find_realNum
gem5_args="--restore-simpoint-checkpoint -r $checkpoint --checkpoint-dir $checkpoint_dir $gem5_args"

curTime=$(date "+%y%m%d-%H%M%S")
#Gem5的输出文件目录，包括stats的输出文件等
gem5_output_dir=$gem5_output_dir/$target/$ISA-$target-"input"$2-"check"$checkpoint-$weight-$curTime
...
#复制checkpoint信息到目标目录
cp $checkpoint_dir/cpt.record $gem5_output_dir/cpt.record
...
echo "checkpoint_dir:"$checkpoint_dir | tee -a $script_out
...
if [ $2 = "0" ]; then
    ...
    cp -r $spec_dir/benchspec/CPU2006/$target/data/test/output/* $run_dir/standout
else
    cp -r $spec_dir/benchspec/CPU2006/$target/data/ref/input/* $run_dir
    cp -r $spec_dir/benchspec/CPU2006/$target/data/ref/output/* $run_dir/standout
fi
