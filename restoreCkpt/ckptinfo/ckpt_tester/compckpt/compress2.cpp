...
void compress_map1(unsigned char *map1, unsigned char *map2, int size)
{
    int len = size>>3;
    for(int i=0; i<len; i++){
        unsigned data = 0;
...
void processFirstLoad2(LoadInfo *linfos, uint64_t loadnum, uint64_t maxsize, FILE *q)
{
    ...
    for(uint64_t i=0;i<loadnum;i++){
        ...
        compress_map1(map1, map2, maxsize/8);
...
void recoveryFirstLoad2(FILE *p)
{
