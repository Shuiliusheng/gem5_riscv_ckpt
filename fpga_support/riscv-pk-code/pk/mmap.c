...
{
  // printk("__vm_alloc current brk: 0x%lx\n", current.brk);
...
{
  // printk("do map enter: 0x%lx\n", addr);
  size_t npage = (length-1)/RISCV_PGSIZE+1;
  if (flags & MAP_FIXED)
  {
    if ((addr & (RISCV_PGSIZE-1)) || !__valid_user_range(addr, length)){
      printk("domap: 0x%lx, %d, %d\n", addr, (addr & (RISCV_PGSIZE-1)), __valid_user_range(addr, length));
      return (uintptr_t)-1;
    }
      
  }
  else if ((addr = __vm_alloc(npage)) == 0){
    printk("domap failed 1\n");
    return (uintptr_t)-1;
  }
...
uintptr_t pk_vm_init()
{
  ...
  printk("memsize: 0x%lx, free_pages: 0x%lx, RISCV_PGLEVEL_BITS: %d\n", mem_size, free_pages, RISCV_PGLEVEL_BITS);
  ...
  // printk("_end: 0x%lx, first_free_page: 0x%lx, first_free_paddr: 0x%lx\n", (uintptr_t)&_end, first_free_page, first_free_paddr);
